...
当前实现将真人会话与机器凭据分开鉴权：JWT 用户认证后进入**空间内角色矩阵**；API Key 则使用独立的 Full Access、Capabilities 和知识库范围策略，不映射为固定的空间成员角色。
...
- **API Key 调用**：`X-API-Key` 表示机器身份，不参与 `viewer < contributor < admin < owner` 角色继承，也不固定等同于 Admin。空间级 Key 的权限由以下策略共同决定：
  - `full_access=true`：可调用所有已经显式声明支持 API Key 的空间级路由；未声明的路由仍默认拒绝。
  - Scoped Key：只可调用与其 `capabilities` 匹配的路由，例如 `retrieve`、`chat`、`ingest` 或各类管理能力。
  - `knowledge_base_ids`：当 Key 配置了知识库范围时，会在 capability 之外继续限制它能够访问的知识库。
  - Platform Key：通过平台级 capability 访问控制面能力，并在请求中选择目标空间；标记为 Platform Only 的路由不接受普通空间级 Key。
...
- **API Key 跨空间访问**：API Key 的 Full Access 或 capabilities 不会自动赋予它其他空间的共享空间成员身份。访问跨空间共享的 KB 时，仍需同时满足 API Key 路由策略、知识库范围和共享关系校验，不能把 API Key 简化理解为“所属空间内的 Admin”。
...
- 或者把脚本切到 `X-API-Key` 调用，并为 Key 配置目标路由所需的 Full Access 或 capability；若设置了 `knowledge_base_ids`，还需确保目标知识库在允许范围内。API Key 不固定等同于 Admin，未声明支持 API Key 的路由仍会返回 403。
